...
!!! info "本包只消费 alpha，不生成信号"
    ...
    MOM / STR / WSTR 这类价格因子属于普通输入，与任何外部 alpha 同等对待。一个完整的构造口径与回测结果见[日频：MOM 12-2 动量](../examples/daily-mom.md)。
...
若价格面板的 `close` 是**未复权原始价**，拆股与缩股会让它跳变。[日频示例](../examples/daily-mom.md#收益口径为何必须复权)记录了一个实测案例：某只股票反向缩股 1:100 后，`close` 从 $0.0383 跳到 $11.50，按价格口径算出 **+25931%** 的假收益，在 493 只等权桶里单独贡献 +64% 的组合收益，使该期多空腿从 −1.76% 变成 −141.17%。
