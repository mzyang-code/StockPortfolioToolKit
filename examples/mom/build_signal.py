"""MOM（12-2 动量）信号生成：全量美股价格面板 → 含 alpha / fwd_ret 列的信号表。

口径与 stock_rag_v1 完全对齐，因子与收益分走两个价格口径：

  因子值   原始 close（未复权）—— 同 Portfolio/benchmarks.py:75-80
      MOM = close[t-21] / close[t-252] - 1     跳过最近 1 个月，只用过去数据
...
def build_signal(
    ...
    # 因子：原始 close，跳过最近一个月
    alpha = grouped["close"].shift(short_lag) / grouped["close"].shift(long_lag) - 1.0
...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成 MOM(12-2 动量) 信号表")
    ...
    parser.add_argument("--long-lag", type=int, default=LONG_LAG)
    parser.add_argument("--short-lag", type=int, default=SHORT_LAG)
    args = parser.parse_args()

    log(f"MOM 信号生成开始 | lag {args.short_lag}/{args.long_lag} | 持有期 {args.holding_days} 日")
    signal = build_signal(
        args.prices, args.first_rebalance, args.stride,
        args.holding_days, args.long_lag, args.short_lag,
    )
    if signal.empty:
