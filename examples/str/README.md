...
未复权时 STR 的 EW 多空腿会算出年化 +485%、净值 ×10 亿的荒谬结果。
...
| 输出信号表 | `/mnt/.../portfolio_tool_cache/signals/signal_str.feather` |
| 回测产物 | `outputs/examples/str/` |
...
−0.055**，即 1 个月反转在市值加权（大盘股主导）口径下基本失效甚至反向 —— 这与文献一致，
短期反转的收益主要来自小盘股的流动性补偿。IC 均值仅 0.0023，是三个因子里最弱的。

## 运行

```bash
# 后台跑全流程（信号 + 回测），日志在 logs/run_str.log
nohup bash examples/str/run.sh > /dev/null 2>&1 &
tail -f examples/str/logs/run_str.log

# 信号已生成，只重跑回测
SKIP_BUILD=1 nohup bash examples/str/run.sh > /dev/null 2>&1 &

# 单步调试
python examples/str/build_signal.py --stride 5 --holding-days 5 --first-rebalance 2021-01-04
python examples/str/run_backtest.py --no-render
```
