"""STR（1 个月短期反转）信号生成：全量美股价格面板 → 含 alpha / fwd_ret 列的信号表。

口径与 stock_rag_v1 完全对齐，因子与收益分走两个价格口径：

  因子值   原始 close（未复权）—— 同 Portfolio/benchmarks.py:75-80
      STR = -(close[t] / close[t-21] - 1)      取负号，过去 1 个月跌得多的排高分位
...
OUT_PATH = "/path/to/portfolio_tool_cache/signals/signal_str.feather"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成 STR(1 个月反转) 信号表")
    ...
    log(f"STR 信号生成开始 | lag={args.lag} | 持有期 {args.holding_days} 日")
    signal = build_signal(
